...
void delay_300ns(unsigned long ns) {
    while (ns > 0) {
        ...
        unsigned int delay_time = _CP0_GET_COUNT() + 12;     // 12 = 300 ns if 1 = 25 ns
...
void delay_1us(unsigned long us) {
    while (us > 0) {
        ...
        unsigned int delay_time = _CP0_GET_COUNT() + 40;    // 40 = 1 microsec if 1 = 25 ns
        while (_CP0_GET_COUNT() < delay_time) {}
        us--;
    }
}
...
// configure display driver to use the second line
